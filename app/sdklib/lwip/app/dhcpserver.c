...
static void ICACHE_FLASH_ATTR wifi_softap_init_dhcps_lease(uint32 ip)
{
	...
	if (dhcps_lease_flag) {
		local_ip = softap_ip = ip;
		softap_ip = ip & 0x00FFFFFF;
		local_ip = ip & 0xFF000000;
		if (local_ip >= 0x80000000)
			local_ip -= DHCPS_MAX_LEASE<<24;
		else
			local_ip +=0x1000000;
...
//	}
	...
	uint8 *sptr = (uint8 *)please;
	uint8 *dptr = &x.c[7];
	int i;
	for(i = 0; i < 8; i++) *dptr-- = *sptr++;
	if((x.dw[0] & x.dw[1]) == 0 || x.dw[1] == x.dw[0] || x.dw[0] - x.dw[1] > DHCPS_MAX_LEASE) return false;
